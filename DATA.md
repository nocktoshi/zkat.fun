...
## Developer vesting (live)

The same response includes a separately validated `devLock` object from `token.creator`, `chainState.vault`, `chainState.beneficiary`, and `chainState.vesting.schedules`. The adapter only interprets current v6 records. BaseStonk’s v6 public token UI formats its dev-buy allocation with 18 token decimals; do not use the NOCK pair’s 16 decimals for ZKAT. Each schedule has `total`, `released`, `start`, `duration`, and `cliff`. The end is `start + duration` in Unix seconds. The display sums scheduled and released quantities with BigInt and uses the source timestamp to label active/future/ended periods. It does not estimate locked wallet balances or label all unreleased tokens as unvested.

Developer and beneficiary wallet addresses are public on-chain addresses, displayed without linking a real-world identity. Links point to their BaseScan address pages. This feature covers the recorded dev-buy vesting allocation, not every asset in the developer wallet and not LP locks. Missing or invalid vesting data shows unavailable while valid rewards can still load. A connection failure retains previous details under a STALE DATA label. A recorded vault/schedule is not presented as an independent security audit.
