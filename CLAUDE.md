...
Keep the highlighted full contract address and Copy CA action immediately above the pool. Keep the trading-tax panel directly below the pool, including live buy/sell rates, the ZKAT paid-to-holders total, and the explanation that holders receive both NOCK and ZKAT from the tax. Keep the live dev-buy vesting panel below that, including creator/vault/beneficiary links, dates, released amount, source timestamp, and honest unavailable/stale states. A public wallet address is intentionally displayed at the owner’s request; do not add the owner’s real-world name or personal contact details. Preserve separate ZKAT (18) and NOCK reward (16 from the source) decimal handling. Do not call this an entire-wallet lock or an LP lock. Keep the unlisted `/flywheel` essay off the homepage and out of any visible navigation. Keep the project X link `https://x.com/zkat_nock` on the site.
